...
| `route_geometries` | One polyline per bus/trolley route (route_id PK, coordinates jsonb). Regenerated by the poller after every static feed import. | tiny |
...
| `latest_snapshot` | One row per entity per period (`hourly`/`daily`/`weekly`/`all`), entity = `route` or `stop`. PK `(period, entity_type, entity_id)`. Built by `agg_snapshot_daily/hourly/weekly/all`. **Frontend reads `latest_snapshot` + `route_geometries`.** |

### Aggregation SQL functions

- `agg_daily(poll_date date)` — aggregates observations into `daily_route_metrics` and `daily_stop_metrics`
- `agg_hourly(poll_date date)` — same but bucketed by hour into `hourly_route_metrics`/`hourly_stop_metrics`. Extracts hour in Eastern time (`AT TIME ZONE 'America/New_York'`).
- `agg_snapshot_daily(poll_date date, now timestamptz)` — upserts today's `daily_*_metrics` rows into `latest_snapshot` (period `daily`)
...
- `agg_snapshot_weekly(now timestamptz)` — sums the last 7 days of `daily_*_metrics` into `latest_snapshot` (period `weekly`)
- `agg_snapshot_all(now timestamptz)` — sums all of `daily_*_metrics` into `latest_snapshot` (period `all`)

All snapshot functions also `DELETE` orphaned rows (no row in the source for the period). Period semantics in the UI: Last Hour = stop arrivals refreshed in the last 60 minutes, Today = today's service date, Last 7 Days = past 7 service dates, All Time = every available service date.

**Scope:** all aggregation is filtered to bus/trolley routes — `route_type IN (0, 3, 11)` (0 = trolley, 3 = bus, 11 = SEPTA's trolleybus-coded routes 59/66/75). Rail (`route_type 1`) is excluded everywhere.

`agg_daily`, `agg_hourly`, and `agg_snapshot_hourly` query `real_time_observations r JOIN stop_times st ON (trip_id, stop_sequence) JOIN trips t ON trip_id JOIN routes r ON route_id` to get route_id (and enforce scope); the daily/weekly snapshots read `daily_*_metrics` instead.
...
2. **Static check** — HEAD request to SEPTA GTFS URL to get `Last-Modified` header. Compare with `service_cycle.last_modified`. If changed, download full zip and re-import all static tables (routes, trips, stops, stop_times, calendar), regenerate `route_geometries`, then record new freshness in `service_cycle`.
...
| `ingestion/poller/route_geometries.py` | Spider-walk route geometry generator (`regenerate_route_geometries()`, `_spider_order()`). Rebuild via `uv run python -m poller.route_geometries` (from `ingestion/`); poller calls it automatically after each static-feed import |
| `ingestion/poller/models.py` | SQLAlchemy models (StopTime, RealTimeObservation, Route, Trip, Stop, Calendar, DailyRouteMetric, HourlyRouteMetric, LatestSnapshot, ServiceCycle, RouteGeometry) |
...
| `frontend/src/composables/useDashboardData.js` | Fetches all periods from `latest_snapshot` and route polylines from `route_geometries` via Neon HTTP, filters client-side by selected period (default `hourly`), polls every 60s |
